...
def getSquaredError(alpha, X_test, Y_test):
    return LA.norm(Y_test - predict(alpha, X_test))
...
def generateReport(X_train, Y_train, X_test, Y_test):
    ...
    report = pd.DataFrame([['alpha_Least_Mean', getSquaredError(alpha, X_test, Y_test)]], columns=headers)
    S = {0.2, 0.4, 0.8, 1.0, 1.2, 1.4, 1.6}
    for s in S:
        s_alpha = LA.inv(X_bias.T @ X_bias + s * np.identity(51)) @ (X_bias.T @ Y_train.T)
        error = getSquaredError(s_alpha, X_test, Y_test)
